...
@extend_schema(
    parameters=[
        OpenApiParameter(
            name="search", description="Search keyword", required=False, type=str
        ),
        OpenApiParameter(
            name="reg_status",
            description="접수예정/접수중/접수마감",
            required=False,
            type=str,
        ),
    ]
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            name="1", description="ID of the Race to review", required=True, type=int
        )
    ],
    examples=[
        OpenApiExample(
            name="Example response",
            value={"author": 1, "race": 1, "contents": "Review content here."},
            request_only=True,
        )
    ],
)
...
def race_reviews(request, race_id):
    ...
    if request.method == "GET":
        ...
        if request.user.is_authenticated:
            serializer = RaceReviewCreateSerializer(
...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            name="사용자명?", description=" 리뷰 ID", required=True, type=int
        )
    ],
    examples=[
        OpenApiExample(
            name="사용자명?",
            value={"author": 1, "race": 1, "contents": "리뷰 내용 수정"},
            request_only=True,
        )
    ],
)
...
def race_top6(request):
    ...
    serializer = RaceListSerializer(
        sorted_races, many=True, context={"request": request}
    )
    return Response(serializer.data)
