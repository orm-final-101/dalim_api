...
class RaceTestCase(TestCase):
    def setUp(self):
        ...
        # 레벨 2개 생성 
        self.level1 = LevelStep.objects.create(number=1, title="Level 1", min_distance=5, max_distance=200)   
        self.level2 = LevelStep.objects.create(number=2, title="Level 2", min_distance=10, max_distance=2000)

        # 유저 2개 생성
        self.user1 = CustomUser.objects.create_user(username="test1", email="test1@test.com", password="test1234!", level=self.level1)
        self.user2 = CustomUser.objects.create_user(username="test2", email="test2@test.com", password="test1234!", level=self.level2)
    ...
    def test_race_top6_list_view(self):
        ...
        print(">> 로그인 상태 확인 없이 요청하면 대회 목록을 반환한다. 메인페이지에 표시 ")
        today = date.today()        
