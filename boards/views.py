...
@extend_schema_view(
    list=extend_schema(
        parameters=[
            ...
            OpenApiParameter(
                name="size", description="x번째 페이지에 게시물 y개", type=int
            ),
            OpenApiParameter(name="category", description="게시물 성격", type=str),
            OpenApiParameter(
                name="post_classification", description="게시물 분류", type=str
            ),
        ]
    )
)
...
def like_post(request, post_id):
    ...
    if request.method == "POST":
        ...
        response_data = {"count": like_count, "is_liked": is_liked}
...
def get_category_choices(request):
    post_classification_choices = [
        {"value": choice[0], "label": choice[1]} for choice in CLASSIFICATION_CHOICES
    ]
    category_choices = [
        {"value": choice[0], "label": choice[1]} for choice in CATEGORY_CHOICES
    ]
