...
class JoinedCrew(models.Model):
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="crews"
    )
    crew = models.ForeignKey(Crew, on_delete=models.CASCADE, related_name="members")
    status = models.CharField(
        max_length=20, choices=CREW_CHOICES, default=CREW_CHOICES[0]
    )
...
class JoinedRace(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    race_record = models.CharField(
        max_length=100, null=True
    )  # 대회 기록. 10:00:00 형식으로 들어감
...
class CustomUser(AbstractUser):
    ...
    gender = models.CharField(
        max_length=6, choices=GENDER_CHOICES, default=GENDER_CHOICES[0]
    )  # 성별. default "none"
    user_type = models.CharField(
        max_length=10, choices=USER_TYPE_CHOICES, default=USER_TYPE_CHOICES[0]
    )  # 유저타입. default "normal"
    location_city = models.CharField(
        max_length=30, choices=LOCATION_CITY_CHOICES, null=True
    )  # 거주지역(시/도)
    ...
    profile_image = models.ImageField(
        upload_to="accounts/profile/%Y/%m/%d/", null=True
    )  # 프로필 이미지
