...
from .permissions import IsCrewMember
...
def crew_join(request, crew_id):
    ...
    if request.data.get("action") == "apply":
        if JoinedCrew.objects.filter(user=user, crew=crew).exists():
            return Response({"error": "이미 가입 신청한 크루입니다."}, status=status.HTTP_400_BAD_REQUEST)
...
def crew_review_list_create(request, crew_id):
    ...
    if request.method == "GET":
        ...
        permission_classes = [IsAuthenticated, IsCrewMember]
        if not all(perm.has_permission(request, crew) for perm in permission_classes):
